...
# Je kan deze poort beschikbaar maken vanop de host met: # "docker run –d –t -p [<host-ip>:]<host-port>:80 <image>" # (vergelijk dit met de port-forwarding bij VirtualBox)
CMD service apache2 start && bash
# Met CMD geef je het commando op dat moet uitgevoerd worden # bij het STARTEN (of RUNNEN) van de CONTAINER. # Er kan slechts één CMD-lijn staan in een Dockerfile. # (Verwar CMD niet met RUN) # (RUN-lijnen worden uitgevoerd bij het BUILDEN van de IMAGE) # (de CMD-lijn wordt uitgevoerd bij het RUNNEN van een CONTAINER) #
# “systemctl” is niet geïnstalleerd op deze Ubuntu, # daarom gebruiken we “service” als alternatief. # De “&& bash” is belangrijk omdat de container anders zou stoppen # na het uitvoeren van “service apache2 start”. # Het commando “bash” blijft immers “hangen”,
